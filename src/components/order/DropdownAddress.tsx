...
interface DropdownAddressProps {
    ...
    setUserPostalCode: React.Dispatch<React.SetStateAction<string>>
    setWarehousePostalCode: React.Dispatch<React.SetStateAction<string>>
    setSelectedWarehouse: React.Dispatch<React.SetStateAction<Warehouse>>
}

export default function DropdownAddress({setSelectedWarehouse, setWarehousePostalCode, setUserAddress, setWarehouseId, setUserPostalCode} : DropdownAddressProps) {
    ...
    const fetchAddressList = useCallback(async () => {
        try {
            ...
            const data = await getAddressList(userData.id);
            setAddressList(data);
            if (data.length > 0) {
                setAddress(data[0].id);
                setUserAddress(data[0].id);
                setUserPostalCode(data[0].postal_code);
            }
        } catch (err) {
            console.log(err);
        }
    ...
    const fetchWarehouseAddress = useCallback(async () => {
        ...
        setWarehousePostalCode(warehouse.postal_code);
